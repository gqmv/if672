...
namespace utils
{
    ...
    bool check_equal(string str, string possible_word, int skip_letters)
    {
        for (int i = skip_letters; i < str.length(); i++)
        {
            if (str[i] != possible_word[i])
                return false;
        }
        return true;
    }
...
int main()
{
    ...
    for (int i = 0; i < consults_count; i++)
    {
        ...
        for (int j = 0; j < possible_locations->size(); j++)
        {
            ...
            string possible_word = text.substr(current_location, word.length());
            if (utils::check_equal(word, possible_word, kmmer_size))
            {
                if (!first)
                {
                    cout << " ";
